...
**Daily Execution Marking ("Executando")**: Coordinators/inspectors can mark areas as "currently being worked on" each morning via the MapInfoCard toggle button ("Marcar Executando" / "Parar Execução"). Marked areas appear with green pulsing markers (#10b981) and can be filtered via the legend "Executando" filter. The `executando` boolean field and `executandoDesde` timestamp are stored in `service_areas` table. All executing areas are **automatically reset at midnight (Brasília time, America/Sao_Paulo)** via server-side scheduler using Intl.DateTimeFormat for reliable timezone handling. Manual reset available via POST `/api/areas/reset-executando`. Toggle endpoint: PATCH `/api/areas/:id/executando` with `{executando: boolean}`.
